...
grouped_data_1=grouped_data[98:]
grouped_data=grouped_data[68:98]
pd.reset_option('display.max_rows')
pd.reset_option('display.max_columns')
grouped_data['count_orignal']=grouped_data['amount']
count_sqrt=np.array(grouped_data['amount'])
...
predict_arima_residuals=(predict_arima_residuals)
result_array =  (np.array(predict_prophet))
series_data=predict_arima[0]
np_predict_arima = series_data.to_numpy()
np_predict_arima=((np_predict_arima))
combined_result=((np.add(result_array,np_predict_arima,predict_arima_residuals)))
